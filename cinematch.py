...
def on_submit():
    ...
    go_back_button  = Button(new_window, text="Go back", command=go_back, style = 'TButton')
    go_back_button.grid(row=6, column=0, columnspan=2, padx=10, pady=10, ipadx=100)

    def display_selected():
        selected_options= genre_entry.curselection()
        inpgenre=[genre_list[i] for i in selected_options]
        display_label.configure(text = "Selected genres: {}".format(", ".join(inpgenre)))
    display_label = Label(new_window, text="",  font = monst14)
    display_label.grid(row=1, column=2, padx=10, pady=10, ipadx=50, sticky=tk.NSEW)

    display_button = Button(new_window, text="Display Selected Genres", command=display_selected)
