...
df=pd.read_excel("C:\\Users\\natas\\.vscode\\python\\mini project_1\\moviesFinal.csv.xlsx")
...
def filter_movies(user_genre, user_year):
        ...
        for i in range(row_count):
            ...
            for j in range(length):
                genre_list.append(row_dict[j]['name'])
            if set(user_genre).issubset(set(genre_list)):
                specresult.append(df['title'].iloc[i])
            else:
                for j in range(length):
...
def on_submit():
    ...
    genre_label.grid(row=1, column=5, padx=10, pady=10, sticky=tk.NSEW)
    ...
    genre_entry.grid(row=1, column=6, padx=10, pady=10, sticky=tk.NSEW)

    year_label = ttk.Label(new_window, text="Enter the year(if you wish):", font=helv16)
    year_label.grid(row=2, column=5, padx=10, pady=10, sticky=tk.NSEW)

    year_entry=ttk.Entry(new_window)
    year_entry.grid(row=2, column=6, padx=10, pady=10, sticky=tk.NSEW)
    ...
    submit_button.grid(row=4, column=5, columnspan=2, padx=10, pady=10, ipadx=100)

    result_label = ttk.Label(new_window, text="")
    result_label.grid(row=3, column=5, columnspan=2, padx=10, pady=10)
